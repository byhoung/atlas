                ...
                $('.ip_slide').each(function( index, value ) {
                    $(this).find('.ip_tooltip').each(function( index, value ){
                        ...
                        switch( type ) {
                            ...
                            if(interaction=="tap") clickHandler($(value), 'ttb-slide');
                            else mouseOverHandler($(value), 'ttb-slide');
                            break;
                            case "btt-slide":
                            insertDescr($(value),'btt-before','pass-btt');
                            //Animation function left to right sliding
                            if(interaction=="tap") clickHandler($(value), 'btt-slide');
                            else mouseOverHandler($(value), 'btt-slide');
                            break;
                            case "btr-slide":
                            insertDescr($(value),'btr-before','pass-btr');
                            //Animation function left to right sliding
                            if(interaction=="tap") clickHandler($(value), 'btr-slide');
                            else mouseOverHandler($(value), 'btr-slide');
                            break;
                            case "rtl-slide":
                            insertDescr($(value),'rtl-before','pass-rtl');
                            //Animation function left to right sliding
                            if(interaction=="tap") clickHandler($(value), 'rtl-slide'); 
                            else mouseOverHandler($(value), 'rtl-slide');
                            break;
                            default:
                            insertDescr($(value), 'ltr-before','pass-ltr');
                            //Animation function left to right sliding
                            if(interaction=="tap") clickHandler($(value), 'ltr-slide');
    ...
    selector.on('tap',function(event){
        ...
        if(prevSelector != selector){
            ...
            if(prevSelector != undefined){
                    hideTooltip(prevSelector, animationType);
                    console.log("switch");
            }
